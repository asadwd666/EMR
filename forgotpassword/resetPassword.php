<?php 
  ...
  $code = $_GET['code'];
  $getEmail = mysqli_query($conn, "SELECT email FROM resetpasswords WHERE code ='$code'");
  ...
  if (isset($_POST['submit'])) {
   ...
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Oops! Passwords did not match! Try aagin </div>";
      ...
      elseif (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
        # code..
        $msg = "<div class='alert alert-danger alert-dismissible'>
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Password must be 8 characters in length with atleast one uppercase and lowercase letter, one numeric and special character <strong>e.g Asadkhan@143<strong></div>";
      }
      else {
       
     $password =sha1($password);
     $query = mysqli_query($conn, "UPDATE users SET password = '$password' WHERE email = '$emailGot'");
     if ($query) {
       # code...
          $query = $conn->query("DELETE FROM resetpasswords WHERE code = '$code'");
          $msg = "<div class='alert alert-success alert-dismissible'>
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Password Updated! successfully <a class='loginHome' href='index.php'>Click to Login</a></div>";
     }
