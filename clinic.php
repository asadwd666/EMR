...
<body>
      ...
      if(mysqli_num_rows($categories) > 0){
        ...
        $categories=mysqli_query($conn,"SELECT * FROM clinical_report WHERE patient_id='$patient_id' and status='unverified'");
        if(mysqli_num_rows($categories) > 0)
        {
          echo "<span>Thank you for submitting your Clinical Record record.A qualified Healthcare profesional will review your details. <br>prior to your record biegn live.approval will be within 48 hours.<br >please chech your support inbox for any additional request from our team.</span> ";
...
$categories=mysqli_query($conn,"SELECT * FROM clinical_report WHERE patient_id='$patient_id' and status='verified'");
...
if(mysqli_num_rows($categories) > 0)
{
  ...
  <a href='profile.php?ptype=check_clinical'>Record</a>
  <span>";
...
$categories=mysqli_query($conn,"SELECT * FROM clinical_report WHERE patient_id='$patient_id' and status='rejected'");
...
if(mysqli_num_rows($categories) > 0)
{
  ...
  Fill Out new Form ?
  <a href='profile.php?ptype=resubmit_clinical'>Add New</a><br>
  .<span>";
